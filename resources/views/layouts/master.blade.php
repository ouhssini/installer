    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-gray-800">{{ config('installer.product.name', 'Application') }}</h1>
                    ...
                    @foreach(['Welcome', 'Requirements', 'Database', 'License', 'Admin', 'Finalize'] as $index => $step)
                        <div class="flex-1 text-center">
                            <div class="relative">
                                ...
                                <div class="text-xs mt-2 {{ ($index + 1) == $currentStep ? 'text-blue-600 font-semibold' : 'text-gray-600' }}">
                                    {{ $step }}
                                </div>
                            </div>
                        </div>
            ...
            <div class="text-center mt-8 text-gray-600 text-sm">
                <p>{{ config('installer.product.name', 'Application') }} v{{ config('installer.product.version', '1.0.0') }}</p>
            </div>
        </div>
    </div>
